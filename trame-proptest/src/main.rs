...
fn arb_shape_recipe(max_existing: usize) -> impl Strategy<Value = ShapeRecipe> {
    prop_oneof![
        ...
        prop::sample::select(vec![1usize, 2, 4, 8]).prop_map(|size| ShapeRecipe::Scalar { size }),
...
fn arb_op_recipe(num_shapes: usize) -> impl Strategy<Value = OpRecipe> {
    prop_oneof![
        // SetField with arbitrary field index (0-7) and source
...
fn run_test(shape_recipes: Vec<ShapeRecipe>, target_shape_idx: usize, op_recipes: Vec<OpRecipe>) {
    with_reset(|| {
...
proptest! {
    ...
    fn arbitrary_shapes_and_ops(
        ...
        op_recipes in prop::collection::vec(
            prop_oneof![
                ((0u8..8), 0usize..6, 0usize..3).prop_map(|(field, shape_idx, kind)| {
                    let src_kind = match kind {
                        0 => SrcKind::Imm { shape_idx },
                        1 => SrcKind::Default,
                        _ => SrcKind::Stage,
                    };
                    OpRecipe::SetField { field, src_kind }
                }),
                (0usize..6, 0usize..3).prop_map(|(shape_idx, kind)| {
                    let src_kind = match kind {
                        0 => SrcKind::Imm { shape_idx },
                        1 => SrcKind::Default,
                        _ => SrcKind::Stage,
